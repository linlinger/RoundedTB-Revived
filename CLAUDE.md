...
- **`Interaction.AddLog` writes to `%LocalAppData%\rtb.log`** — unlike upstream, its body is enabled (writes timestamped lines; logging failures are swallowed). `Background` also logs throttled `bw[...]` and `hover:` diagnostic lines. `Debug.WriteLine` output goes to the debugger only.
...
- **Config schema migration**: `Interaction.ReadJSON` populates over first-launch defaults (absent keys keep their defaults instead of dropping to 0/false), falls back to defaults on a corrupt or empty file, and `MigrateLegacySettings` carries pre-3.0 configs (`CornerRadius`/`MarginBasic` sentinel `-384` spread across all segments, `ShowTrayOnHover` → `ShowSegmentsOnHover`) over to the per-segment schema — old users don't need to re-enable anything. Writes are atomic (temp file + `File.Replace`). `MainWindow` still keeps its own null-layout guards as a second line of defence.
