...
- [ ] **动态模式边缘 padding 用户可调**(2026-08-15 记):当前硬编码 `6*ScaleFactor`
  (防圆角裁切图标,Taskbar.cs UpdateDynamicTaskbar)。做成设置项,并用**滑块拖动实时预览**
  (目前边距是输入框 + 点"应用"才生效,用户希望动态感知、不用点应用就看效果)。
- [ ] **圆角抗锯齿**(2026-08-15 记):`SetWindowRgn` 的圆角是 1-bit 位图,无抗锯齿
  (Windows region 固有限制,原版 README 已知问题已注明),边缘有像素毛刺。真正圆滑需改为
  **WS_EX_LAYERED + 自绘带 alpha 的圆角位图**(渲染架构大改、风险高,待规划)。轻量缓解:
  圆角不要超过任务栏高度的一半(否则弧线变形、锯齿更明显)。
